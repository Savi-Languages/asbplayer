jest.mock('@project/common/src/image-transformer', () => ({
    cropAndResize: jest.fn(async () => 'data:image/jpeg;base64,/9j/2Q=='),
}));
...
const setup = () => {
    ...
    video.getBoundingClientRect = () =>
        ({ left: 10, top: 10, width: 300, height: 200, right: 310, bottom: 210 }) as DOMRect;
    return video;
};
...
test('drops the image when the scene changes during capture; save can continue without it', async () => {
    ...
    const send = jest.fn(async () => {
        current = false;
        return { dataUrl: 'data:image/jpeg;base64,/9j/2Q==' };
    });
...
test('never captures an offscreen or hidden player', async () => {
    const video = setup();
    video.getBoundingClientRect = () =>
        ({ left: -50, top: 0, width: 300, height: 200, right: 250, bottom: 200 }) as DOMRect;
    const send = jest.fn();
    expect(await captureWatchScreenshot(video, send, () => true)).toBeUndefined();
