...
export function buildGlossHtml(segments: GlossSegment[], glossFor: (lemma: string) => string | undefined): string {
    let html = '';
    let glossed = false;
    for (const seg of segments) {
...
export class SaviGlossController implements GlossProvider {
    ...
    /**
     * Re-read the roaming target language, on EVERY line rather than once per
     * bind.
     *
     * `start()` races the background's cloud refresh at video load, and a
     * language switched in the app mid-watch never reached a running binding at
     * all — a stale "fr" once glossed half an episode as all-"untokenized",
     * fixable only by a page refresh. A storage.local read per line is nothing;
     * self-healing is not.
     *
     * An empty value is ignored rather than applied: the cache is briefly empty
     * before the first cloud load, and blanking a good language would undo the
     * very staleness fix this exists for.
     */
    private async _refreshTargetLang(): Promise<void> {
        const { targetLanguage } = await getCachedRoamingSettings();
    ...
    private async _computeLine(text: string, priority: boolean): Promise<void> {
        ...
        try {
            // Refresh BEFORE segmenting, not just before the gloss request.
            // `_glossLine` re-reads the roaming language per call (0.46.1, for
            // the stale-'fr' bug), but tokenization now depends on the language
            // too — SV-33 made the stoplist and the capitalize-every-noun rule
            // per-language — and it runs here, one step earlier. Without this,
            // the first line after a mid-watch language switch would still be
            // segmented under the OLD language's rules: German nouns dropped as
            // names, or Spanish function words filtered out of French.
            await this._refreshTargetLang();
            const segments = segmentLine(text, this._targetLang);
            const candidates = glossCandidates(segments);
            if (candidates.length === 0) {
                this._why('no-candidates', 'no candidates — every word was punctuation, a function word, or a name');
    ...
    private async _glossLine(line: string, words: string[], priority: boolean): Promise<Map<string, string>> {
        const cached = words
        ...
        try {
            ...
            if (skipped.length === response.words.length) {
                ...
                if (response.words.some((w) => w.skip === 'no-analyzer')) {
                    ...
                    this._why('all-skipped', `all ${skipped.length} word(s) skipped — ${skipSummary(response.words)}`);
                }
            }
