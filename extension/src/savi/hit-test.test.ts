...
describe('boxesEqual', () => {
    it('tolerates sub-pixel jitter but not a real move or a different row count', () => {
        const a = [box(0, 0, 10, 12), box(0, 20, 10, 12)];
        expect(boxesEqual(a, [box(0.4, 0, 10, 12.3), box(0, 20.5, 10, 12)])).toBe(true);
        expect(boxesEqual(a, [box(1, 0, 10, 12), box(0, 20, 10, 12)])).toBe(false);
        expect(boxesEqual(a, [box(0, 0, 10, 12)])).toBe(false);
        expect(boxesEqual(a, null)).toBe(false);
    });
});
...
// above the cue, it sits at cue-top − gap (48px floor) − arrow(7).
describe('SaviHoverDictionary hover — boxes and popup on a wrapped word', () => {
    ...
    it('boxes both rows of the word and puts the popup above the whole cue', async () => {
        ...
        // which would have parked the popup on the first row. The gap is the
        // 48px floor (a 12px row × 1.8 is less), plus the arrow.
        expect(popupTop()).toBe(`${504 - 48 - 7}px`);
