...
export default class SubtitleReader {
    ...
    // Parses the Netflix IMSC 1.1 (TTML) shape seen in subtitle downloads on the
    // state-tree capture path: tick-based timestamps resolved against ttp:tickRate,
    // and furigana as tts:ruby styles (a container span wrapping a base span and a
    // text span). Furigana renders inline as base(reading) so the existing
    // netflixRubyRegex / _convertNetflixRubyToHtml pass tokenizes it.
    ...
    // Flattens an IMSC <p> to text, expanding ruby containers to base(reading).
    ...
    private _imscRubyText(container: Element, rubyRoleOf: (element: Element) => string | undefined): string {
        ...
        // Furigana as base(reading) — the reader's netflixRubyRegex pass tokenizes it.
        return `${base}(${reading})`;
    }
